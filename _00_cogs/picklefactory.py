...
"""
from _02_global_dicts import *
varsToSave = {"player_dict" : player_dict, "resource_dict" : resource_dict, "region_dict" : region_dict, "district_dict" : district_dict, "played_cards_dict" : played_cards_dict}
"""
...
class PickleFactory(commands.Cog):
    ...
    async def loadAll(self, ctx):
        global played_cards_dict

        player_dict.clear()
        resource_dict.clear()
        region_dict.clear()
        district_dict.clear()
        played_cards_dict.clear()
        ...
        #reinstating districts
        for key in district_dict.keys():
            district_dict[key].reinstate(self.bot.get_guild(district_dict[key].guildID))

        with open(f"{os.getcwd()}\\_01_pickle_jar\\played_cards_dict.pkl", "rb") as file:
            played_cards_dict = pickle.load(file)
